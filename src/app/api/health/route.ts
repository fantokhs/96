...
export async function GET() {
  ...
  if (!supabaseConfigured() && !memoryStoreAllowed()) {
    return json({ ok: false, version: "V1.3", store: "none", env, error: "Supabase environment variables are missing" }, 503);
  }
  try {
    ...
    return json({ ok, version: "V1.3", store: store.kind, env, categories: categories.length, questions: questions.length }, ok ? 200 : 503);
  } catch (e) {
    return json({ ok: false, version: "V1.3", env, error: (e as Error).message }, 503);
  }
}
